...
class TestBazelBotServer(unittest.TestCase):
    ...
    @mock.patch("utils.git.Repo")
    @mock.patch("os.path.exists")
    def test_git_push_fix_uses_retry_for_subops(
        ...
        self.assertEqual(call2[0][0], (utils.git.GitCommandError,))
    ...
    def test_refresh_main_branch_uses_retry(
        self, mock_exists, github_mock, auth_mock, mock_repo, mock_sleep
    ):
        mock_exists.return_value = False
        creds = mock.MagicMock()
        creds.gh_fork_repo_name = "fork/repo"
        ...
        self.assertEqual(repo.gh_fork_repo.merge_upstream.call_count, 2)
        self.assertEqual(mock_sleep.call_count, 1)
        repo_instance.remotes.origin.fetch.assert_called_once()
